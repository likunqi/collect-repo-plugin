...
1. Use agent-reach skill for GitHub search (via agent-reach gh backend):
   Run 2-3 search queries via `gh search repos`
   ```
   gh search repos "<keywords>" --sort stars --limit 5 --json fullName,stargazersCount,description,url
   ```
...
- Step 7: use agent-reach skill (gh backend) for GitHub search; Node.js https for Notion dedup check
